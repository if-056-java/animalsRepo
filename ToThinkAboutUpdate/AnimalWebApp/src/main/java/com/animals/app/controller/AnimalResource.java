...
public class AnimalResource {
	...
	@Consumes(MediaType.APPLICATION_JSON)
	@Produces({MediaType.APPLICATION_JSON, MediaType.APPLICATION_XML})
	public List<Animal> insertAnimal (Animal animal) {
	...
	@PUT //not ready
	@Path("animal/{animalId}") //http:localhost:8080/AnimalWebApp/webapi/animals/animal/id
	@Consumes(MediaType.APPLICATION_JSON)
	@Produces({MediaType.APPLICATION_JSON, MediaType.APPLICATION_XML})
	public Animal update (Animal animal, @PathParam ("animalId") String animalId) {
		
		int idi = (int) Integer.parseInt(animalId);
		
		anRep.update(animal, idi);
		
		Animal updatedAnimal = anRep.getById(idi);
